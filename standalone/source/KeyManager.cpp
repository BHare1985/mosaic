...
bool KeyManager::LoadKeys(const std::string& publicKeyPath)
{
    cert_ = zcert_load(publicKeyPath.c_str());
    return cert_ != nullptr;
}

bool KeyManager::GenerateKeys(const std::string& certificateFilename)
{
    cert_ = zcert_new();
    if (cert_ == nullptr) {
        return false;
    ...
    std::cout << "Public/Private key pair generated successfully." << std::endl;
...
bool KeyManager::GenerateKeysFromPrivate( const std::string& privateKey){
